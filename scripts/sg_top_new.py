...
@contextmanager
def temp_debug(mode: str, outdir: str):
    dbg0, out0 = pcv.params.debug, pcv.params.debug_outdir
    try:
        pcv.params.debug = mode
        pcv.params.debug_outdir = outdir
        yield
    finally:
        pcv.params.debug = dbg0
        pcv.params.debug_outdir = out0
# ========================== CONFIG ==========================
# I/O
INPUT_PATH   = r"C:\Cantonese\topview_test.jpg"   # ใส่ไฟล์เดี่ยว หรือโฟลเดอร์
OUTPUT_DIR   = r".\results_topsingle_new"
...
def _analyze_plant_metrics(
    plant_mask, rw, rh,
    do_prune=DO_PRUNE, fallback_no_prune=FALLBACK_NO_PRUNE,
    debug_dir=None, plant_id=None
    ):
...
def segment(rgb_img, filename):
    ...
    # เตรียมโฟลเดอร์ debug ต่อไฟล์ภาพสำหรับ per-plant
    perplant_dir = os.path.join(pcv.params.debug_outdir, "perplant", Path(filename).stem)
    os.makedirs(perplant_dir, exist_ok=True)

    metrics_by_pid = {
        pid: _analyze_plant_metrics(
            pm, rw_slot, rh_slot,
            debug_dir=perplant_dir, plant_id=pid
        )
        for pid, pm, _ in plants
    }
