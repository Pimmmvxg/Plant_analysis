...
def overlay_image(img_bgr, mask_u8, alpha=0.5, color=(0,255,0)):
    if img_bgr is None or mask_u8 is None:
        raise ValueError("img_bgr หรือ mask_u8 เป็น None")
    
    if mask_u8.ndim == 3:
        mask_u8 = cv2.cvtColor(mask_u8, cv2.COLOR_BGR2GRAY)
        
    if img_bgr.shape[:2] != mask_u8.shape[:2]:
        raise ValueError("ขนาด img / mask ไม่ตรงกัน")
    ...
    overlay_color = np.empty_like(out)
    overlay_color[:] = color
    out[mask_bool] = (
        out[mask_bool] * (1.0 - alpha) + overlay_color[mask_bool] * alpha
    ).astype(np.uint8)
